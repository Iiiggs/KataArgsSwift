...
public class Parser
{
    ...
    subscript(index:String) -> AnyObject? {
        if (index == "l"){
            ...
            return 8080 as AnyObject
        }
        else if (index == "d") {
            return "/usr/logs" as AnyObject
        }
        else {
            return nil
        }
    ...
    init(input:String){
        for flag in input.components(separatedBy: "-"){
            let flag_unpacked = flag.components(separatedBy: " ")
            let flag_name = flag_unpacked[0]
            let flag_value = flag_unpacked[1]?
            // how to know type from scheme?
            
            // check if values exist
            
            // unpack the type
            
            if let value = flag_value {
